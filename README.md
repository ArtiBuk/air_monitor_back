...
- Городской фон в `air-map` и при сборке датасета строится из объединения `open_meteo + plumelabs`: исторические данные Plume сохраняются, а новые часы закрываются Open-Meteo.
- Сбор наблюдений работает в режиме частичной деградации: если один из провайдеров временно недоступен (например, `502` у Plume), backend продолжает сбор из остальных источников и возвращает предупреждения в ответе `/observations/collect`.
