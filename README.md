...
Дополнительно backend формирует человекочитаемый итоговый PDF-отчёт по текущему состоянию системы: наблюдения, источники данных, датасет, рабочая модель, последний прогноз, проверка качества и итоговые выводы.
...
- snapshot для интерактивной карты качества воздуха;
- генерация итогового PDF-отчёта для демонстрации результатов;
...
Если менялась генерация итогового отчёта или логика PDF, достаточно перезапустить `web`, потому что именно он отдает файл `GET /api/monitoring/overview/report.pdf`.
...
- `GET /api/monitoring/air-map` отдает snapshot для карты воздуха: последние station-level точки `MyCityAir`, городской фон `Plume Labs` и границы карты.
- `GET /api/monitoring/overview/report.pdf` собирает краткий итоговый отчёт в формате PDF.
- Для кириллического PDF backend использует встроенные в репозиторий шрифты `apps/monitoring/assets/fonts/Arial.ttf` и `ArialBold.ttf`, поэтому рендер не зависит от системных шрифтов контейнера.
